...
class FileReaderChunk
{
  ...
  static function readAll(string $filename, int $chunkSize = 8192): Promise
  {
    ...
    self::readChunk($stream, $buffer, $chunkSize, $deferred);

    return $promise;
  }

  private static function readChunk($stream, string &$buffer, int $chunkSize, Deferred $deferred): void
  {
    ...
    Loop::delay(0, function() use ($stream, &$buffer, $chunkSize, $deferred) {
      self::readChunk($stream, $buffer, $chunkSize, $deferred);
    });
  }
}